...
class Pool extends ActiveRecord {
  ...
  static function get_pool_posts_from_posts($posts) {
    ...
    $pool_posts = PoolPost::find_by_sql(array($sql, $post_ids));
    return $pool_posts;
  }
  
  static function get_pools_from_pool_posts($pool_posts) {
    ...
    // $pools = 
    // $pools = Pool::find_by_sql($sql, array($pool_ids), array('return_array' => true));
    $pools = self::find_by_sql(array($sql, $pool_ids));
    // vde($pools);
    return $pools;
  }
