...
if (Request::$post) {
  ...
  if (!empty(Request::$params->ids)) {
    foreach (array_keys(Request::$params->ids) as $post_id) {
      ...
      $post = Post::$_->find($post_id);
      
      if (Request::$params->commit == "Approve")
        $post->approve(User::$current->id);
      elseif (Request::$params->commit == "Delete") {
        $post->destroy_with_reason(Request::$params->reason ? Request::$params->reason : Request::$_->params->reason2, User::$current);
  ...
  if (!empty(Request::$params->query)) {
    $pending_posts = Post::$_->find_by_sql(Post::$_->generate_sql(Request::$params->query, array('pending' => true, 'order' => "id desc")));
    $flagged_posts = Post::$_->find_by_sql(Post::$_->generate_sql(Request::$params->query, array('flagged' => true, 'order' => "id desc")));
  } else {
    $pending_posts = Post::$_->find('all', array('conditions' => "status = 'pending'", 'order' => "id desc"));
    $flagged_posts = Post::$_->find('all', array('conditions' => "status = 'flagged'", 'order' => "id desc"));
  }
}
?>